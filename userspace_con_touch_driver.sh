#!/bin/bash
#
# Script communicating directly with the Raspberry Pi 7 inch touchscreen display (but probably
# not only this LCM), and forwarding touch events to the console as ASCII art representation.
# No need for X, absolutely headless.
#
...
#
# This script requires i2c-tools (sudo apt-get install i2c-tools)
#
# Original author: Simon Gustafsson (www.optisimon.com)
# Copyright (c) 2016 Simon Gustafsson (www.optisimon.com)
#
# Improvements and adaptation for headless usage by Nazar Gerasymchuk (trola.org) 2019
#
# Do whatever you like with this code, but please refer to the original author(s).
#
...
while true ; do
  ...
  if [ "${NUM_TOUCHES}" == "1" ] ; then
    ...
    if [ $RELEASED == "1" ] ; then
      ...
      ./cursor_emulation.sh $CTP_SIZE ${X} ${Y} "CLICK"
    else
